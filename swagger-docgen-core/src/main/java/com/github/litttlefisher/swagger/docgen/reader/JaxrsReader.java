...
import com.github.litttlefisher.swagger.docgen.jaxrs.BeanParamInjectParamExtension;
import com.github.litttlefisher.swagger.docgen.jaxrs.JaxrsParameterExtension;
...
public class JaxrsReader extends AbstractReader implements ClassSwaggerReader {
    ...
    protected Swagger read(Class<?> cls, String parentPath, String parentMethod, boolean readHidden,
        ...
        for (Method method : filteredMethods) {
            ...
            if (operationPath != null) {
                ...
                apiConsumes = parserOperationConsumes(parentConsumes, apiConsumes, operation);
                apiProduces = parserOperationProduces(parentProduces, apiProduces, operation);

                handleSubResource(apiConsumes, httpMethod, apiProduces, tags, method, apiOperation, operationPath,
                    operation);
    ...
    private void handleSubResource(String[] apiConsumes, String httpMethod, String[] apiProduces, Map<String, Tag> tags,
        Method method, ApiOperation apiOperation, String operationPath, Operation operation) {
        if (isSubResource(httpMethod, method)) {
            Class<?> responseClass = method.getReturnType();
            if (apiOperation != null && !apiOperation.response().equals(Void.class) && !apiOperation.response().equals(
                void.class)) {
                responseClass = apiOperation.response();
            }
            LOGGER.debug("handling sub-resource method " + method.toString() + " -> " + responseClass);
            read(responseClass, operationPath, httpMethod, true, apiConsumes, apiProduces, tags,
    ...
    protected boolean isSubResource(String httpMethod, Method method) {
        Class<?> responseClass = method.getReturnType();
        return (responseClass != null) && (httpMethod == null) && (AnnotationUtils.findAnnotation(method, Path.class)
            != null);
    }
    ...
    public Operation parseMethod(String httpMethod, Method method) {
        ...
        if ((responseClassType != null) && !responseClassType.equals(Void.class) && !responseClassType.equals(
            void.class) && !responseClassType.equals(javax.ws.rs.core.Response.class) && !hasApiAnnotation
            ...
            if (isPrimitive(responseClassType)) {
                ...
                if (models.isEmpty()) {
                    ...
                    operation.response(responseCode, new Response().description("successful operation").schema(p)
        ...
        for (int i = 0; i < parameterTypes.length; i++) {
            ...
            for (Parameter parameter : parameters) {
                if (hasCommonParameter(parameter)) {
                    Parameter refParameter = new RefParameter(
                        RefType.PARAMETER.getInternalPrefix() + parameter.getName());
    ...
    private Parameter replaceArrayModelForOctetStream(Operation operation, Parameter parameter) {
        if (parameter instanceof BodyParameter && operation.getConsumes() != null && operation.getConsumes().contains(
            ...
            if (schema instanceof ArrayModel) {
                ...
                if ((items != null) && "byte".equals(items.getFormat()) && ("string".equals(items.getType()))) {
